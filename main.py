...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [
        [InlineKeyboardButton("Plano 1", callback_data="plano1")],
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(
        f"Olá {user.first_name}, envie o comprovativo para {MEU_TELEGRAM}"
    )
...
async def receber_comprovativo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"Comprovativo recebido! O administrador ({MEU_TELEGRAM}) irá confirmar."
    )
